...
# for gride size and number of initial initial states
m = int(numbers[0])
n = int(numbers[1])
k = int(numbers[2])
...
# to store position of flipped cells
positions1 = []
positions2 = []
        ...
        for i in range(1, n+1):
            for j in range(1, m+1):
                ...
                if int(check) == 1:
                    ...
                    y = j
                    tup = (x, y)
                    positions1.append(tup)
                else:
                    x = i
                    y = j
                    tup = (x, y)
                    positions2.append(tup)
        # making changes for cells that are 
        # flipped for the next iteration
        for tup in positions1:
            x = tup[0]
            y = tup[1]
            grid[x][y] = 1
        positions1.clear()
        for tup in positions2:
            x = tup[0]
            y = tup[1]
            grid[x][y] = 0
        positions2.clear()
